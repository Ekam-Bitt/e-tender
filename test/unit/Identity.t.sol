...
import { ZKNullifierVerifier } from "src/identity/ZKNullifierVerifier.sol";
...
contract IdentityTest is Test {
    ...
    ZKNullifierVerifier zkNullifierVerifier;
    Halo2NullifierVerifier halo2Nullifier;
    ...
    function setUp() public {
        ...
        // ZKNullifier Setup
        vm.prank(authority);
        halo2Nullifier = new Halo2NullifierVerifier();
        zkNullifierVerifier = new ZKNullifierVerifier(address(halo2Nullifier));
    ...
    function testZKNullifierVerification_RejectsFakeProof() public {
        // Attempt to verify a garbage proof
        bytes32[] memory signals = new bytes32[](3);
        signals[0] = bytes32(uint256(1)); // commitment
        signals[1] = bytes32(uint256(2)); // nullifier
        signals[2] = bytes32(uint256(3)); // external_nullifier

        // Long enough but invalid proof
        bytes memory longFakeProof = new bytes(192); // Min length

        vm.expectRevert();
        zkNullifierVerifier.verify(longFakeProof, signals);
    }
}
